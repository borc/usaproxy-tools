...
** Special version without inline dependencies, modified by Teemu Pääkkönen **
...
(function($) {
	var options = {
		...
		zoom : {
			...
			trigger : "dblclick", // or "click" for single click
			amount : 1.5
		// how much to zoom relative to current position, 2 = 200% (zoom in),
		// 0.5 = 50% (zoom out)
		},
		pan : {
			interactive : false,
	...
	function init(plot) {
		...
		plot.zoom = function(args) {
			...
			var c = args.center, amount = args.amount
					|| plot.getOptions().zoom.amount, w = plot.width(), h = plot
					.height();
			...
			$
					.each(
							plot.getAxes(),
							function(_, axis) {
								var opts = axis.options, min = minmax[axis.direction].min, max = minmax[axis.direction].max, zr = opts.zoomRange;
								...
								var range = max - min;
								if (zr
										&& ((zr[0] != null && range < zr[0]) || (zr[1] != null && range > zr[1])))
									return;
		...
		plot.pan = function(args) {
			...
			$.each(plot.getAxes(), function(_, axis) {
				...
				min = axis.c2p(axis.p2c(axis.min) + d), max = axis.c2p(axis
						.p2c(axis.max)
						+ d);
