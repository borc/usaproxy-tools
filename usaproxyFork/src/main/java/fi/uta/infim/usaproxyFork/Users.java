...
public class Users {
	...
	private Hashtable<String, ArrayList<String>>		proposals;		
	...
	public Users() {
		...
		proposals 	= new Hashtable<String, ArrayList<String>>();
	...
	public void send (OutputStream out, String topic, String sid) throws IOException {
		 ...
		 *  referenced by the proposer's session ID */
			...
			ArrayList<String> pproposals;
			if (proposals.containsKey(sid))
				pproposals = proposals.get(sid);
			else
				pproposals = new ArrayList<String>(); 
	...
	public void acceptProposal(OutputStream out, String accepter, String proposer) throws IOException {
		...
		for (int i = 0; i < admins.size(); i++) {
			...
			if (!admins.get(i).equals(accepter) && proposals.containsKey(admins.get(i))) {
				ArrayList<String> pproposals = proposals.get(admins.get(i));
	...
	private void checkFinalAck(OutputStream out, String accepter, String proposer, int timeouts) throws IOException {
		...
		if(accepterArray[1].equals("false")) {
			...
			/** if proposer isn't available anymore */
				...
				ArrayList<String> pproposals = proposals.get(accepter);
		...
		if (accepter==null) {
			for (int i = 0; i < admins.size(); i++) {
				...
				if (proposals.containsKey(admins.get(i))) {
					pproposals = proposals.get(admins.get(i));
	...
	public void isProposalAccepted(OutputStream out, String proposer, String accepter) throws IOException {
		...
		if (accepter!=null) {
			...
			if (partners.containsKey(proposer) 
				...
				&& proposals.get(accepter).contains(proposer)) {
		...
		if (!containsValueInArraylists(proposals, accepter)) {
			...
			/** sb) accepter specified */
				...
				if (proposals.containsKey(accepter))
					pproposals = proposals.get(accepter);
				else
					pproposals = new ArrayList<String>();
	 ...
	 */
	protected boolean containsValueInArraylists(Hashtable<String, ArrayList<String>> hashtable, String value) {
		
		for (Iterator<Entry<String, ArrayList<String>>> e = hashtable.entrySet().iterator(); e.hasNext();) {
			
	        Entry<String, ArrayList<String>> me 		= e.next();
	        String user 	= (String) me.getKey();
	        if (null == user) continue;
	        ArrayList<String> arraylist = me.getValue();
	 ...
	 */
	public Hashtable<String, ArrayList<String>> getProposals() {
		return proposals;
	}
