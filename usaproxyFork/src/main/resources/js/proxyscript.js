...
function startLogging_UsaProxy(invokedOnLoad /*boolean*/) {
	...
	if ((FLG_RemoteMonitoring && remoteMonitorer=="false") || !FLG_RemoteMonitoring) {
		...
		var trackedElements = jQuery_UsaProxy( 'p:visible, div:visible, img:visible, span:visible, h1:visible, h2:visible, h3:visible, h4:visible, h5:visible, h6:visible' );
		var screenID = 1;
		...
		{
			writeLog_UsaProxy( "appear&nodeName=" + this.nodeName + "&nodeId=" + this.id + "&screenID=" + screenID + generateEventString_UsaProxy( this ) );
		} );
		var previousUpdateTime = jQuery_UsaProxy.now();
		
		// Check element visibility when the page is scrolled or the window resized.
		jQuery_UsaProxy( window ).on( 'scroll resize', function()
		{
			++screenID;
		
			// No log writing will be done unless THRESHOLD amount of milliseconds
			// has passed. This value should be as high as possible while still
			// being low enough to not allow the user to scroll a whole screenful
			// during the time. If the user scrolls a whole screenful, some
			// 'appear' events and the respective 'disappear' events can be 
			// erroneously left unlogged.
			var THRESHOLD = 200;
			if ( jQuery_UsaProxy.now() - previousUpdateTime < THRESHOLD ) return;
			
			jQuery_UsaProxy( elementsPreviouslyInViewport )
				.filter( ':not( :in-viewport )' )
				.each( function()
				{
					writeLog_UsaProxy( "disappear&nodeName=" + this.nodeName + "&nodeId=" + this.id + "&screenID=" + screenID + generateEventString_UsaProxy( this ) );
				} );
			
			var elementsCurrentlyInViewport = jQuery_UsaProxy( trackedElements ).filter( ':in-viewport' );
			jQuery_UsaProxy( elementsCurrentlyInViewport )
				.not( elementsPreviouslyInViewport )
				.each( function()
				{
					writeLog_UsaProxy( "appear&nodeName=" + this.nodeName + "&nodeId=" + this.id + "&screenID=" + screenID + generateEventString_UsaProxy( this ) );
				} );
				
			elementsPreviouslyInViewport = elementsCurrentlyInViewport;
